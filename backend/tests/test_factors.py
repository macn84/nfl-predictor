...
class TestBettingLines:
    ...
    def test_oddspapi_used_first_when_key_set(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "app.prediction.factors.betting_lines.settings.oddspapi_api_key",
            "fake-key",
        )
        monkeypatch.setattr("app.prediction.factors.betting_lines.settings.odds_api_key", "")
        calls: list[str] = []
        ...
        monkeypatch.setattr(
            "app.prediction.factors.betting_lines._fetch_oddspapi", fake_fetch_oddspapi
        )
        result = betting_lines.calculate("KC", "BUF")
    ...
    def test_falls_back_to_odds_api_when_oddspapi_fails(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "app.prediction.factors.betting_lines.settings.odds_api_key",
            "fake-odds-key",
        )
