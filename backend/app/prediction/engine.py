...
def _run_factors(
    ...
    raw: list[FactorResult] = [
        form.calculate(
            schedules, team_stats, home_team, away_team, week, season, game_date=game_date
        ),
...
def predict(
    ...
    normalized = _run_factors(
        home_team, away_team, season, schedules, team_stats, game_date, settings.weights
    )
...
def predict_cover(
    ...
    Uses a separate weight profile (cover_weights from settings) intended for
    ...
    they cannot affect production output until you set them. The merged
