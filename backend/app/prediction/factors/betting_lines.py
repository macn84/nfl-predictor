...
def _discover_oddspapi_nfl_ids() -> tuple[int, int] | None:
    ...
    if (
        _oddspapi_discovery_failed_at
        and (time.time() - _oddspapi_discovery_failed_at) < _DISCOVERY_RETRY_SECONDS
    ):
        return None
    ...
    for sport in sports:
        ...
        if (
            "american football" in name
            or "american" in name and "football" in slug
            or "nfl" in slug
        ):
...
def calculate(
    ...
    - Current/upcoming game → The Odds API first, OddspaPI as fallback
