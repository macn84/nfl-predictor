  ...
  const cardContent = (
    <>
      ...
      <div className="mt-3 pt-3 border-t border-app-border space-y-2">
        {/* Verdict badge — DISAGREE only. AGREE / NO_DATA and legacy FADE/BOOST show nothing. */}
        {isAuthenticated && llm?.verdict === 'DISAGREE' && (
          <span className="inline-block text-xs font-mono font-semibold border rounded px-1.5 py-0.5 leading-none text-app-red border-app-red/40">
            DISAGREE{llm.impact_pts != null ? ` (~${Math.abs(llm.impact_pts).toFixed(1)} pts)` : ''}
          </span>
        )}
        {/* Explain + evidence — only when the AI found a fact that invalidates the pick */}
        {isAuthenticated && llm?.verdict === 'DISAGREE' && llm.explain && (
          <p className="text-xs text-app-muted leading-relaxed">
            {llm.explain}
          </p>
        )}
        {isAuthenticated && isUpcoming && llm?.verdict === 'DISAGREE' && llm.evidence?.length > 0 && (
          <ul className="text-xs text-app-gold leading-relaxed border-l-2 border-app-gold/40 pl-2 space-y-0.5">
